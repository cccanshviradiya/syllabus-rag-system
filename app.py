...
#             st.warning("⚠️ Gemini unavailable. Falling back to local Phi-3.")
#             return ask_phi3(context, question)

        
#       answer = ask_llm_with_fallback(context,question)
...
#         st.write(answer)
#     except Exception:
#         st.error("⚠️ Ollama is not responding. Make sure `phi3` is running.")


# # -------------------- STREAMLIT UI --------------------
# def main():
#     st.set_page_config(page_title="Chat PDF")
#     st.header("📘 Syllabus RAG System (Phi-3 + OCR)")
...
def ask_llm_with_fallback(context, question):
    try:
        ...
        st.warning("⚠️ Gemini failed. Falling back to local Phi-3.")
        return ask_phi3(context, question)
...
def main():
    st.set_page_config(page_title="Chat PDF")
    st.header("📘 Syllabus RAG System (Gemini + Phi-3 fallback)")
